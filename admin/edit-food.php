        ...
        <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select name="category_id" id="category_id" class="form-control">
                    ...
                    ?>
                            <option value="<?php echo $cat['id'] ?>" <?php if ($data['category_id'] == $cat['id']) echo 'selected' ?>><?php echo $cat['title'] ?></option>
                        <?php
                        }
                    } else {
                        ?>
            ...
            <div class="form-group">
                ...
                <div>
                    ...
                    <img style="width: 100px; height: 100px;" src="<?php echo SITEURL; ?>admin/uploads/food/<?php echo $data['image_name']; ?>" alt="">
            ...
            <div class="form-group">
                ...
                <textarea name="description" id="description" style="width: 420px;" rows="4"><?php echo $data['description'] ?></textarea>
            </div>
            <div class="form-group">
...
if (isset($_POST['submit'])) {
    ...
    if (!empty($_FILES['image_name']['name'])) {
        if (isset($_FILES['image_name']['name'])) {
            ...
            if (file_exists('uploads/food/' . $old_image) && !empty($old_image)) {
                // Ready to Unlink image
                $remove = unlink('uploads/food/' . $old_image);
    ...
    if ($title != "" && $featured != "" && $active != "" && $price != "" && $description != "") {
        // Create SQL query for insert data into database
        $sql = "UPDATE tbl_food SET category_id='$category_id', title='$title', image_name='$image_unique_name', description='$description', price='$price', featured='$featured', active='$active' WHERE id=$id";
        // Execute the query
        $res = $connection->query($sql);
        ...
        if ($res == true) {
            ...
            // Redirect to page
            echo ("<script>location.href='" . SITEURL . "admin/edit-food.php'</script>");
            // Stop to process
            die();
        }
    } else {
